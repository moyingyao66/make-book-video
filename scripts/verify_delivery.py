#!/usr/bin/env python3
"""Independently fail closed unless every delivery artifact is still valid."""
...
def project_file(project: Path, value: Any, label: str, errors: list[str]) -> Path | None:
    try:
        return secure_project_path(project, value, f"delivery artifact {label}")
...
def load_object(path: Path, label: str, errors: list[str]) -> dict[str, Any]:
    try:
        ...
        errors.append(f"delivery artifact {label} is invalid JSON: {exc}")
        return {}
    if not isinstance(document, dict):
        errors.append(f"delivery artifact {label} JSON root must be an object")
        return {}
    return document
...
def independent_media_report(
    ...
    if missing_tools:
        errors.append("delivery verification tools are missing: " + ", ".join(missing_tools))
        return report
    ...
    try:
        ...
        errors.append(f"delivery video ffprobe returned invalid JSON: {exc}")
        return report
    ...
    if len(video_streams) != 1:
        errors.append("delivery video must contain exactly one video stream")
    if len(audio_streams) != 1:
        errors.append("delivery video must contain exactly one audio stream")
    ...
    if video_stream.get("codec_name") != "h264":
        errors.append("delivery video codec is not H.264")
    if (video_stream.get("width"), video_stream.get("height")) != (1080, 1920):
        errors.append("delivery video dimensions are not 1080x1920")
    if abs(frame_rate - 30.0) > 0.01:
        errors.append("delivery video frame rate is not 30 fps")
    if audio_stream.get("codec_name") != "aac":
        errors.append("delivery audio codec is not AAC")
    if str(audio_stream.get("sample_rate") or "") != "48000":
        errors.append("delivery audio sample rate is not 48 kHz")
    try:
    ...
    if expected_frames <= 0 or actual_frames != expected_frames:
        errors.append("delivery video frame count differs from build report")
    try:
        expected_duration = float(build.get("durationSeconds") or 0)
    except (TypeError, ValueError):
        expected_duration = 0.0
    if expected_duration <= 0 or abs(duration - expected_duration) > 0.12:
        errors.append("delivery video duration differs from build report")
    ...
    if decode_result.returncode != 0:
        errors.append(
            "delivery video full decode failed: "
    ...
    try:
        ...
        errors.append(f"delivery audio packet verification failed: {exc}")
    else:
        ...
        if mix_packet_hash != video_packet_hash:
            errors.append("delivery video audio packets differ from the approved audio mix")
    return report
...
def verify_contact_sheets(
    ...
    for relative in FIXED_CONTACT_SHEETS:
        try:
            path = secure_project_file(project, relative, f"delivery review sheet {relative}")
            width, height = inspect_png(path)
        except (ProjectArtifactError, ValueError, OSError) as exc:
            errors.append(f"delivery review sheet is missing or invalid: {exc}")
            continue
        fixed[relative] = path
        report["files"].append(
            {
    ...
    if str(qa_report.get("contactSheet") or "") != FIXED_CONTACT_SHEETS[0]:
        errors.append("delivery QA contactSheet must use the fixed review sheet path")
    if str(qa_report.get("boundaryContactSheet") or "") != FIXED_CONTACT_SHEETS[1]:
        errors.append("delivery QA boundaryContactSheet must use the fixed review sheet path")
    ...
    try:
        ...
        errors.append(f"delivery review-sheet timeline is malformed: {exc}")
        return report
    if duration <= 0 or fps <= 0 or not starts:
        errors.append("delivery review sheets require positive media timing and scene boundaries")
        return report
    ...
    with tempfile.TemporaryDirectory(prefix=".verify-contact-") as temporary:
        ...
        for label, result in (
            ...
            if result.returncode != 0:
                errors.append(
                    f"delivery independent {label} sheet extraction failed: "
        ...
        try:
            ...
            errors.append(f"delivery independently extracted review sheet is invalid: {exc}")
            return report
        comparisons = (
        ...
        for relative, generated in comparisons:
            if sha256(fixed[relative]) != sha256(generated):
                errors.append(
                    f"delivery review sheet does not match the current video: {relative}"
                )
    report["ok"] = not any(
...
def verify_build_inputs(
    ...
    try:
        ...
        build_version = 0
    if build_version < 4:
        errors.append("delivery build report version is unsupported")
    if build.get("status") != "rendered-pending-human-review":
        errors.append("delivery build report status is invalid")
    for marker_path, marker_hash, build_path, build_hash in (
        ...
        if str(build.get(build_path) or "") != str(marker.get(marker_path) or ""):
            errors.append(f"delivery build report {build_path} path differs from marker")
        if str(build.get(build_hash) or "") != str(marker.get(marker_hash) or ""):
            errors.append(f"delivery build report {build_hash} differs from marker")
    fixed_inputs = (
    ...
    for hash_field, relative in fixed_inputs:
        try:
            path = secure_project_file(project, relative, f"delivery frozen input {relative}")
        except ProjectArtifactError as exc:
            errors.append(str(exc))
            continue
        if str(build.get(hash_field) or "") != sha256(path):
            errors.append(f"delivery build report {hash_field} is missing or stale")
    timed_inputs = (
    ...
    for path_field, hash_field in timed_inputs:
        ...
        if path is None:
            continue
        if not path.is_file():
            errors.append(f"delivery build input is missing: {path_field}")
    ...
    errors.extend("delivery " + error for error in inventory_errors)
    if marker.get("renderInputInventory") != recorded_inventory:
        errors.append("delivery marker renderInputInventory differs from build report")
...
def verify_human_review(
    ...
    if human.get("passed") is not True:
        errors.append("delivery human review is not passed")
    for field in ("reviewedAt", "reviewer"):
        if not str(human.get(field) or "").strip():
            errors.append(f"delivery human review {field} is missing")
        if str(marker.get(field) or "") != str(human.get(field) or ""):
            errors.append(f"delivery marker {field} differs from human review")
    if not is_timezone_aware_iso8601(human.get("reviewedAt")):
        errors.append("delivery human review reviewedAt must be timezone-aware ISO-8601")
    if str(human.get("reviewedArtifact") or "") != str(marker.get("video") or ""):
        errors.append("delivery human review identifies a different video path")
    if str(human.get("videoSha256") or "") != str(marker.get("videoSha256") or ""):
        errors.append("delivery human review video hash differs from marker")
    if str(human.get("editableDeliverySha256") or "") != str(
        marker.get("editableDeliverySha256") or ""
    ):
        errors.append("delivery human review editable hash differs from marker")
    ...
    if not isinstance(checks, dict):
        errors.append("delivery human review checks must be an object")
        checks = {}
    for field in required_checks:
        if checks.get(field) not in (True, "passed"):
            errors.append(f"delivery human review check is missing or not passed: {field}")

    evidence_values = human.get("evidence")
    if not isinstance(evidence_values, list) or not evidence_values:
        errors.append("delivery human review evidence list is missing")
        evidence_values = []
    ...
    for index, value in enumerate(evidence_values, start=1):
        ...
        if not path.is_file():
            errors.append(f"delivery human evidence is missing: {relative}")
    marker_evidence = marker.get("humanEvidence")
    if not isinstance(marker_evidence, list):
        errors.append("delivery marker humanEvidence must bind every reviewed evidence file")
        marker_evidence = []
    bound_paths: list[str] = []
    for index, item in enumerate(marker_evidence, start=1):
        if not isinstance(item, dict):
            errors.append(f"delivery marker humanEvidence[{index}] must be an object")
            continue
        ...
        if not path.is_file():
            errors.append(f"delivery marker human evidence is missing: {relative}")
        elif str(item.get("sha256") or "") != sha256(path):
            errors.append(f"delivery marker human evidence hash is stale: {relative}")
    if evidence_paths != bound_paths or len(set(bound_paths)) != len(bound_paths):
        errors.append("delivery marker humanEvidence differs from human review evidence")
    for required in FIXED_CONTACT_SHEETS:
        if required not in evidence_paths:
            errors.append(f"delivery human review must include fixed evidence: {required}")
        if required not in bound_paths:
            errors.append(f"delivery marker must bind fixed human evidence: {required}")
    if qa_report.get("humanEvidence") != marker_evidence:
        errors.append("delivery QA humanEvidence differs from delivery marker")
    for field, required in zip(
        ("contactSheet", "boundaryContactSheet"), FIXED_CONTACT_SHEETS
    ):
        if str(qa_report.get(field) or "") != required:
            errors.append(f"delivery QA {field} must use the fixed evidence path")
        elif required not in evidence_paths:
            errors.append(f"delivery QA {field} was not included in human review evidence")
    if qa_report.get("visualReview") != human:
        errors.append("delivery QA visualReview differs from the bound human review ledger")


def validate_delivery(project: Path) -> dict[str, Any]:
    project = project.resolve()
    marker_path = project / "renders/qa/delivery-ready.json"
    errors: list[str] = []
    try:
        marker_path = secure_project_file(
            project, "renders/qa/delivery-ready.json", "delivery marker"
        )
    except ProjectArtifactError as exc:
        return {"ok": False, "marker": str(marker_path), "errors": [str(exc)]}
    marker = load_object(marker_path, "delivery marker", errors)
    if not marker:
        return {"ok": False, "marker": str(marker_path), "errors": errors}
    if marker.get("version") != 2:
        errors.append("delivery marker version must be 2")
    if marker.get("contract") != "make-book-video-delivery-v1":
        errors.append("delivery marker contract is unsupported")
    if marker.get("ready") is not True:
        errors.append("delivery marker does not declare ready=true")
    ...
    for path_field, hash_field in ARTIFACT_FIELDS:
        ...
        if path is None:
            continue
        if not path.is_file():
            errors.append(f"delivery artifact is missing: {path.relative_to(project)}")
            continue
        artifact_paths[path_field] = path
        actual_hash = sha256(path)
        if str(marker.get(hash_field) or "") != actual_hash:
            errors.append(f"delivery artifact hash is stale: {hash_field}")
        verified[path_field] = {
    ...
    for field in ("editorRoute", "editorProjectId", "editorTimelineId"):
        if not str(marker.get(field) or "").strip():
            errors.append(f"delivery marker {field} is required")
    ...
    delivery_case: dict[str, Any] = {}
    delivery_manifest: dict[str, Any] = {}
    try:
        delivery_case_path = secure_project_file(project, "case.json", "delivery case.json")
        delivery_manifest_path = secure_project_file(
            project, "render-manifest.json", "delivery render-manifest.json"
        )
        delivery_case = load_object(delivery_case_path, "case.json", errors)
        delivery_manifest = load_object(
            delivery_manifest_path, "render-manifest.json", errors
        )
        try:
            case_version = int(delivery_case.get("version") or 0)
        except (TypeError, ValueError):
            case_version = 0
        if case_version < 3:
            errors.append("delivery requires case.version >= 3")
        if delivery_case and delivery_manifest:
            delivery_contract_errors = validate_case(
                delivery_case,
                require_approved=True,
                project=project,
                manifest=delivery_manifest,
            )
            delivery_contract_errors.extend(
                validate_manifest(
                    project,
                    delivery_case,
                    delivery_manifest,
            ...
            errors.extend(
                ...
                for error in delivery_contract_errors
            )
    except (ProjectArtifactError, OSError, ValueError, TypeError, AttributeError) as exc:
        errors.append(f"delivery case/manifest validation failed safely: {exc}")
    ...
    if build:
        try:
            ...
            errors.append(f"delivery provider timing verification failed: {exc}")
        else:
            if independent_timing.get("ok") is not True or timing_failures:
                errors.extend(
                    "delivery provider timing: " + str(error)
    ...
    if editable:
        if editable.get("status") != "verified":
            errors.append("delivery editable delivery is not verified")
        for marker_field, editable_field in (
            ...
            if str(marker.get(marker_field) or "") != str(editable.get(editable_field) or ""):
                errors.append(f"delivery {marker_field} differs from editable delivery")
        try:
            editable_validation = validate_editable_delivery(project, editable, strict=True)
        except (SystemExit, Exception) as exc:
            errors.append(f"delivery editable delivery validation failed: {exc}")
        else:
            if not editable_validation.get("ok"):
                errors.extend(
                    "delivery editable delivery: " + str(error)
    ...
    if editor_plan:
        try:
            ...
            errors.append(f"delivery editor plan replay failed: {exc}")
        else:
            if editor_plan != expected_editor_plan:
                errors.append("delivery editor plan differs from deterministic replay")
            if editor_plan.get("status") != "planned-not-executed":
                errors.append("delivery editor plan status must be planned-not-executed")
            if editor_plan.get("editorExecutionClaimed") is not False:
                errors.append("delivery editor plan must not claim editor execution")
    ...
    if qa_report:
        if qa_report.get("ok") is not True:
            errors.append("delivery QA report does not declare ok=true")
        if qa_report.get("structuralOk") is not True:
            errors.append("delivery QA report does not declare structuralOk=true")
        if qa_report.get("decodePassed") is not True:
            errors.append("delivery QA report does not declare decodePassed=true")
        if qa_report.get("humanReviewPending") not in (False, None):
            errors.append("delivery QA report still declares human review pending")
        failures = qa_report.get("failures")
        if not isinstance(failures, list) or failures:
            errors.append("delivery QA report failures must be an empty array")
        qa_video_value = qa_report.get("video")
        if not isinstance(qa_video_value, dict):
            errors.append("delivery QA video must be an object")
            qa_video_value = {}
        if str(qa_video_value.get("sha256") or "") != str(
            marker.get("videoSha256") or ""
        ):
            errors.append("delivery QA report video hash differs from marker")
        ...
        if media_report:
            ...
            for field, actual in expected_video_values.items():
                if qa_video.get(field) != actual:
                    errors.append(f"delivery QA video {field} differs from ffprobe")
            try:
            ...
            if abs(qa_fps - float(media_report.get("fps") or 0)) > 0.001:
                errors.append("delivery QA video fps differs from ffprobe")
            if abs(
                qa_duration - float(media_report.get("durationSeconds") or 0)
            ) > 0.001:
                errors.append("delivery QA video duration differs from ffprobe")
        qa_audio_value = qa_report.get("audio")
        if not isinstance(qa_audio_value, dict):
            errors.append("delivery QA audio must be an object")
            qa_audio_value = {}
        qa_audio = qa_audio_value
        if qa_audio.get("packetHashMatches") is not True:
            errors.append("delivery QA report does not prove the approved audio mix")
        if str(qa_audio.get("mixSha256") or "") != str(marker.get("audioMixSha256") or ""):
            errors.append("delivery QA report audio mix hash differs from marker")
        if str(qa_audio.get("mix") or "") != str(marker.get("audioMix") or ""):
            errors.append("delivery QA report audio mix path differs from marker")
        if media_report:
            if qa_audio.get("codec") != media_report.get("audioCodec"):
                errors.append("delivery QA audio codec differs from ffprobe")
            if str(qa_audio.get("sampleRate") or "") != str(
                media_report.get("audioSampleRate") or ""
            ):
                errors.append("delivery QA audio sample rate differs from ffprobe")
        qa_timing = qa_report.get("providerTiming")
        if not isinstance(qa_timing, dict) or qa_timing.get("ok") is not True:
            errors.append("delivery QA report providerTiming is not verified")
        elif qa_timing.get("failures") not in (None, []):
            errors.append("delivery QA report providerTiming still contains failures")
        if independent_timing and qa_timing != independent_timing:
            errors.append("delivery QA providerTiming differs from independent verification")
        qa_editable = qa_report.get("editableDelivery")
        if not isinstance(qa_editable, dict) or qa_editable.get("ok") is not True:
            errors.append("delivery QA report editableDelivery is not verified")
        elif editable_validation:
            for field in ("route", "projectId", "timelineId"):
                if qa_editable.get(field) != editable_validation.get(field):
                    errors.append(f"delivery QA editableDelivery {field} is stale")
        if build and qa_report.get("renderInputInventory") != build.get(
            "renderInputInventory"
        ):
            errors.append("delivery QA renderInputInventory differs from build report")
        qa_plan = qa_report.get("editorPlan")
        if not isinstance(qa_plan, dict):
            errors.append("delivery QA editorPlan must be an object")
        else:
            if str(qa_plan.get("path") or "") != str(marker.get("editorPlan") or ""):
                errors.append("delivery QA editor plan path differs from marker")
            if str(qa_plan.get("sha256") or "") != str(
                marker.get("editorPlanSha256") or ""
            ):
                errors.append("delivery QA editor plan hash differs from marker")
            if qa_plan.get("status") != "planned-not-executed":
                errors.append("delivery QA editor plan status is invalid")
            if qa_plan.get("editorExecutionClaimed") is not False:
                errors.append("delivery QA editor plan incorrectly claims editor execution")
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "ok": False,
            "marker": str(
                args.project.resolve() / "renders/qa/delivery-ready.json"
            ),
            "errors": [
                "delivery verification failed safely on malformed evidence: "
